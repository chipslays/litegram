    ...
    public function middleware(...$args)
    {
        ...
        if (count($args) == 1) {
            foreach ((array) $args[0] as $key) {
                $this->canContinueEvent = isset($this->middlewares[$key]) ? $this->call($this->middlewares[$key]) : false;
         ...
         */
        if (count($args) == 2) {
            return isset($this->middlewares[$args[0]]) ? $this->call($this->middlewares[$args[0]], [$args[1]]) : false;
        }
    }
